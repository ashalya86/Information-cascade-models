...
Cascade14 changed as deterministic model
@author: srias292
"""
...
def plot_graph(
    ...
    plt.ylabel("Probability Ratio of Cascade")
    plt.title(
        ...
        if j > 0:
            ...
            def func_sum_upper_half_and_middle(prob_dist):
                sum_of_upper_half = sum(
                    [
                        prob_dist[j]
                        for j in range(math.floor(len(prob_dist) / 2), len(prob_dist),)
                    ]
                )
...
n = 100
list_p = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.95]
runs = 1000
real_v = 1
num_prior_agents = 1
